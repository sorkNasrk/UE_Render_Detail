...
**[尚未验证]**先保存 A 的工程副本和固定观察条件，再建立明确命名的 Nanite 单项对照。A 原本关闭项目支持，因此先在项目设置开启 `r.Nanite.ProjectEnabled=1`，保存、重启并等待相关 Shader 编译完成；再确认运行开关 `r.Nanite=1`，给适用静态资产启用并构建 Nanite。只改变 Nanite 这一功能轴，保持光照、阴影、材质系统、相机和输出条件不变。它用于观察 Nanite 自身行为，不把它另称为完整 B。确认后再回到 Substrate、Lumen、VSM 全部启用的 B。
