...
本表覆盖总览与第 01～28 章。解释用于快速找回上下文，不替代正文算法与源码解读。英文大小写通常不影响概念含义，但 C++ 符号的大小写需要与源码一致。
...
| 中文／标识 | 英文 | 用途与区别 |
|---|---|---|
| 调试／性能分析 | Debugging / Profiling | 分别定位结果错误与工作或等待的成本；所需证据不同 |
| 语义缓冲可视化 | Buffer Visualization | 通过材质解码和映射表面通道，不保证显示显存原始字节 |
| 资源版本 | Resource Version | 在具体图或捕获中某次写入后的内容，不是全局帧号 |
| 包含／自身时间 | Inclusive / Exclusive Time | 包含子事件／排除归入子事件的工作，不能把两者随意累加 |
| 经过／忙碌时间 | Elapsed / Busy Time | 起止差／统计定义下的工作时间；须说明工具、列与队列口径 |
| 等待／空闲 | Wait / Idle | 依赖未满足或尚无可执行工作；具体分类按事件实现解释 |
| 关键路径 | Critical Path | 约束结果最早完成的依赖链；缩短被完全重叠的工作未必缩短整帧 |
| 事件跟踪／书签 | Trace / Bookmark | 记录连续事件流／标记实验区间；未启用通道可能没有对应证据 |
| 预热／稳定区间 | Warm-up / Steady-state Interval | 将首次编译、流送、缓存建立与代表性稳定负载分开统计 |
| 图形帧捕获 | GPU Frame Capture | 保存 API 或资源状态以分析特定帧，不代表未附加工具的长期性能 |
| Shader 调试符号 | Shader Debug Symbols | 协助定位程序与源码；需要适用编译产物，不能即时补出所有缓存程序的符号 |

[第 26～27 章](../chapters/26-basic-frame-walkthrough.md) 把这些资料接回 P/Q 的完整帧，[第 28 章](../chapters/28-debugging-and-profiling.md) 提供工具选择、命令、恢复步骤和记录模板。相关实现定位见 [源码索引](source-index.md)，概念补充阅读见 [参考资料](references.md)。
